...
function isValidExpiryMonth(month, options) {
  var settings = Hoek.applyToDefaults(defaults.expiryMonths, options || {});
...
function isValidExpiryYear(year, options) {
  var settings = Hoek.applyToDefaults(defaults.expiryYears, options || {});
...
function doesNumberMatchType(number, type, options) {
  var settings = Hoek.applyToDefaults(defaults.cardTypes, options || {});
...
function _setupCardTypeAliases(type, aliases) {
  for (var i = 0, il = aliases.length; i < il; ++i) {
    defaults.cardTypes[aliases[i]] = defaults.cardTypes[type]
  }
}
