...
function validate (card, options) {
  ...
  const cardType = Hoek.reach(card, schema.cardType);
  const number = sanitizeNumberString(Hoek.reach(card, schema.number));
  const expiryMonth = Hoek.reach(card, schema.expiryMonth);
  const expiryYear = Hoek.reach(card, schema.expiryYear);
  const cvv = sanitizeNumberString(Hoek.reach(card, schema.cvv));
